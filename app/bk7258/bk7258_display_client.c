/****************************************************************************
 * app/bk7258/bk7258_display_client.c
 *
 * SPDX-License-Identifier: Apache-2.0
 *
 * CP-side client for the application-owned BKDisplay RPMsg protocol.
 ****************************************************************************/

#include <nuttx/config.h>

#ifdef CONFIG_BK7258_APP_DISPLAY
...
struct bkdisplay_client_s
{
  ...
  struct bkdisplay_rpc_response_s reply;
};

static struct bkdisplay_client_s g_bkdisplay_client =
{
...
static void bkdisplay_client_flush_sem(sem_t *sem)
{
...
static bool bkdisplay_client_endpoint_ready(
  struct bkdisplay_client_s *client)
{
  bool ready = false;
...
static int bkdisplay_client_wait_endpoint(
  struct bkdisplay_client_s *client)
{
  clock_t start = clock_systime_ticks();
  clock_t limit = MSEC2TICK(BKDISPLAY_RPC_ENDPOINT_WAIT_MS);
  int ret;

  do
    {
      if (bkdisplay_client_endpoint_ready(client))
        {
          return 0;
        }
...
static int bkdisplay_client_send_bounded(
  struct bkdisplay_client_s *client,
  const struct bkdisplay_rpc_request_s *request)
{
  clock_t start = clock_systime_ticks();
  clock_t limit = MSEC2TICK(BKDISPLAY_RPC_SEND_WAIT_MS);
  int ret = -ENOTCONN;
...
static int bkdisplay_client_wait_reply(
  struct bkdisplay_client_s *client,
  const struct bkdisplay_rpc_request_s *request,
...
static int bkdisplay_client_cb(struct rpmsg_endpoint *endpoint, void *data,
                               size_t len, uint32_t src, void *priv)
{
  struct bkdisplay_client_s *client = priv;
  const struct bkdisplay_rpc_response_s *response = data;
  irqstate_t flags;
  bool matched;
  ...
  if (response == NULL || len != sizeof(*response) ||
      ...
      response->reserved != 0 ||
      memchr(response->expression, '\0',
             sizeof(response->expression)) == NULL ||
      memchr(response->pack_id, '\0', sizeof(response->pack_id)) == NULL)
    {
      return -ENOMSG;
    }
...
static void bkdisplay_client_device_created(struct rpmsg_device *rdev,
                                            void *priv)
{
  struct bkdisplay_client_s *client = priv;
...
static void bkdisplay_client_device_destroy(struct rpmsg_device *rdev,
                                            void *priv)
{
  struct bkdisplay_client_s *client = priv;
  const char *cpuname = rpmsg_get_cpuname(rdev);
...
{
  struct bkdisplay_client_s *client = &g_bkdisplay_client;
  ...
  if (ret >= 0)
    {
      ...
      ret = rpmsg_register_callback(client,
                                    bkdisplay_client_device_created,
                                    bkdisplay_client_device_destroy,
                                    NULL, NULL);
    }
                           ...
                           unsigned int timeout_ms)
{
  ...
  ret = bkdisplay_rpc_client_initialize();
  if (ret < 0)
    {
      return ret;
    }
  ...
  ret = bkdisplay_client_wait_endpoint(client);
  if (ret < 0)
    {
      goto out;
    }
  ...
  bkdisplay_client_flush_sem(&client->reply_sem);
  flags = spin_lock_irqsave(&client->reply_lock);
  ...
  ret = -ETIMEDOUT;
  for (attempt = 0; attempt < BKDISPLAY_RPC_ATTEMPTS; attempt++)
    {
      ret = bkdisplay_client_send_bounded(client, request);
      if (ret < 0)
        {
          break;
        }

      ret = bkdisplay_client_wait_reply(client, request, response,
                                        timeout_ms);
      if (ret != -ETIMEDOUT)
        {
          break;
...
#endif /* CONFIG_BK7258_APP_DISPLAY */
