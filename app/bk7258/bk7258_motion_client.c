/****************************************************************************
 * app/bk7258/bk7258_motion_client.c
 *
 * SPDX-License-Identifier: Apache-2.0
 *
 * CP-side client for the application-owned BKMOTION RPMsg protocol.
 ****************************************************************************/
...
static struct bkmotion_client_s g_bkmotion_client =
{
...
static void bkmotion_flush_sem(sem_t *sem)
{
...
static bool bkmotion_endpoint_ready(struct bkmotion_client_s *client,
                                    uint32_t *epoch)
{
  bool ready = false;
...
static int bkmotion_wait_endpoint(struct bkmotion_client_s *client,
                                  uint32_t *epoch)
{
  clock_t start = clock_systime_ticks();
  clock_t limit = MSEC2TICK(BKMOTION_RPC_ENDPOINT_WAIT_MS);
  int ret;

  do
    {
      if (bkmotion_endpoint_ready(client, epoch))
        {
          return 0;
        }
...
static int bkmotion_send_bounded(
  struct bkmotion_client_s *client,
  const struct bkmotion_rpc_request_s *request, uint32_t epoch)
{
  clock_t start = clock_systime_ticks();
  clock_t limit = MSEC2TICK(BKMOTION_RPC_SEND_WAIT_MS);
  int ret = -ENOTCONN;
...
static int bkmotion_wait_reply(
  struct bkmotion_client_s *client,
  const struct bkmotion_rpc_request_s *request,
  struct bkmotion_rpc_response_s *response, unsigned int timeout_ms,
  uint32_t epoch)
{
  clock_t start = clock_systime_ticks();
...
static int bkmotion_client_cb(struct rpmsg_endpoint *endpoint, void *data,
                              size_t len, uint32_t src, void *priv)
{
  struct bkmotion_client_s *client = priv;
  const struct bkmotion_rpc_response_s *response = data;
  irqstate_t flags;
  bool matched;
  ...
  if (response == NULL || len != sizeof(*response) ||
      ...
      !bkmotion_rpc_response_valid(response))
    {
      return -ENOMSG;
    }
...
static void bkmotion_connection_lost(struct bkmotion_client_s *client)
{
  irqstate_t flags;
...
static void bkmotion_client_unbind(struct rpmsg_endpoint *endpoint)
{
  struct bkmotion_client_s *client = endpoint->priv;

  if (nxmutex_lock(&client->endpoint_lock) >= 0)
    {
      bkmotion_connection_lost(client);
      nxmutex_unlock(&client->endpoint_lock);
    }
}

static void bkmotion_device_created(struct rpmsg_device *rdev, void *priv)
{
  struct bkmotion_client_s *client = priv;
  ...
  if (!__atomic_load_n(&client->endpoint_created, __ATOMIC_ACQUIRE))
    {
      ...
      ret = rpmsg_create_ept(&client->endpoint, rdev, BKMOTION_RPC_ENDPOINT,
                             RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
                             bkmotion_client_cb, bkmotion_client_unbind);
      __atomic_store_n(&client->connection_error, ret, __ATOMIC_RELEASE);
      if (ret >= 0)
        {
...
{
  struct bkmotion_client_s *client = &g_bkmotion_client;
  ...
  if (ret >= 0)
    {
      ...
      ret = rpmsg_register_callback(client, bkmotion_device_created,
                                    bkmotion_device_destroy, NULL, NULL);
    }
                          ...
                          unsigned int timeout_ms)
{
  ...
  ret = bkmotion_rpc_client_initialize();
  if (ret < 0)
    {
      return ret;
    }
  ...
  ret = bkmotion_wait_endpoint(client, &epoch);
  if (ret < 0)
    {
      goto out;
    }
  ...
  bkmotion_flush_sem(&client->reply_sem);
  flags = spin_lock_irqsave(&client->reply_lock);
  ...
  ret = -ETIMEDOUT;
  for (attempt = 0; attempt < BKMOTION_RPC_ATTEMPTS; attempt++)
    {
      ret = bkmotion_send_bounded(client, request, epoch);
      if (ret < 0)
        {
          break;
        }

      ret = bkmotion_wait_reply(client, request, response, timeout_ms, epoch);
      if (ret != -ETIMEDOUT)
        {
          break;
        }
...
#endif /* CONFIG_BK7258_APP_MOTION */
